    ...
    <b-row class="mt-3">
      ...
      <b-col cols='12' md='6'>
        <div class="product-info border rounded p-3">
          ...
          <b-row class="sellers border p-3 mt-3 align-items-center justify-content-between">
            <h6 class="m-0">Seller: 
              <nuxt-link 
              class="seller-link"
              :to='`/user/${seller}`' v-for="seller in sellers" :key="seller">
                {{seller}}
              </nuxt-link>
            </h6>
            <div class="stars">
            <b-icon v-for="star in Array(randomStar)" :key="star"
            icon='star-fill' class="text-warning"></b-icon>
            <span v-if="remainStars > 0">
              <b-icon v-for="star in Array(remainStars)" :key='star'
              icon='star' class="text-warning"></b-icon>
            </span>
